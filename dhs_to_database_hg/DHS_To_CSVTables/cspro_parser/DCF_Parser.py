...
class DCF_Parser:
    ...
    def parse(self, expand_ranges="All", range_expansion_limit=10000,
        ...
        with open(self._dcf_filename, encoding=self.detect_encoding()) as fileIn:
                    ...
                    if skippingChunk:
                            ...
                            if currentLevelName in myLevels:
                                if myLevels[currentLevelName] == currentLevelLabel:
                                    conversion_issue_warnings.extend(
                                        [
                                            "Duplicate level name/label encountered at line {}".format(
                                                str(self._parsed_lines))
                                        ]
                                    )
                                    # print("Duplicate level name/label encountered at line {}".format(
                                    #         str(self._parsed_lines)))
                                else:
                                    raise ValueError(
                                        "Duplicate level name encountered at line {0!s} with non-matched label".format(
                                            self._parsed_lines)
                        ...
                        # so the item will have already been added to the output list myItems
                            ...
                            if not (simRatio > 0.7 or chunkInfo['Label'].find(myItems[-1]['Label']) == 0):
                                conversion_issue_warnings.extend(
                                    [
                                        "Valueset did not seem to match item at line {} of file {} - please check!".format(
                                            self._parsed_lines, self._dcf_filename)
                                    ]
                                )
                                # print("Valueset did not seem to match item at line {} of file {} - please check!".format(
                                #         self._parsed_lines, self._dcf_filename))
                            ...
                            if 'ValueRanges' in chunkInfo:
                                ...
                                for rangeInfo in chunkInfo['ValueRanges']:
                                    try:
                                        ...
                                        conversion_issue_warnings.extend([self._parsed_lines])
                    ...
                    if currentlyParsing == 'Relation':
                        ...
                        if match:
                            try:
                                ...
                                conversion_issue_warnings.extend(
                                    ["uhoh! fieldVal={} || chunkInfo={}".format(fieldVal, chunkInfo)]
                                )
                                # print("uhoh! fieldVal={} || chunkInfo={}".format(fieldVal, chunkInfo))
            ...
            # print("Parsed {} lines into {} items".format(self._parsed_lines, len(myItems)))

            self._items = myItems
            self._relations = myRelations
            self._parsed = True

        return conversion_issue_warnings
